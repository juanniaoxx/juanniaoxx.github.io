...
---
!!! abstract "主要内容"
	一维差分：通过差分数组高效处理区间增减，将区间操作转化为端点修改，前缀和还原结果。  
	等差数列差分：将等差数列转化为差分序列（一阶差分为常数，二阶差分为0），简化区间更新。
	二维前缀和：预处理矩阵前缀和，快速查询子矩阵和，公式为`S[i][j] = S[i-1][j] + S[i][j-1] - S[i-1][j-1] + A[i][j]`。  
	二维差分：扩展至矩阵，通过四个角点操作实现子矩阵增减，二维前缀和还原结果。    
