...
\begin{tcolorbox}[title=两两独立与相互独立]
    ...
    leftbrace/.style={decorate, decoration={brace, amplitude=10pt, mirror}}, % "{" 开口朝下
    rightbrace/.style={decorate, decoration={brace, amplitude=10pt}}         % "}" 开口朝上
    ]

    % 四行内容节点
    \node (line1) at (0,3) {$P(AB)=P(A)P(B)$};
    \node (line2) at (0,2) {$P(AC)=P(A)P(C)$};
    \node (line3) at (0,1) {$P(BC)=P(B)P(C)$};
    \node (line4) at (0,0) {$P(ABC)=P(A)P(B)P(C)$};

    % 左边的大括号 "{"，包含全部四行
    \draw [leftbrace] (-3,3) -- (-3,0) node[midway, left=12pt] {};

    % 右边的大括号 "}"，只包含前三行（从第1行到第3行）
    \draw [rightbrace] (2,3) -- (2,1) node[midway, right=12pt] {};
